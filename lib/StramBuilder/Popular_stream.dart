...
import '../helper/popular_product_helper.dart';
import '../model/popular_product_model.dart';
import '../product/DetailPageForViewProduct.dart';
class PopularStream extends StatefulWidget {

  const PopularStream({Key? key}) : super(key: key);


  @override
  State<PopularStream> createState() => _PopularStreamState();
}

class _PopularStreamState extends State<PopularStream> {
  ...
  Widget _buildListView(List<DocumentSnapshot> documents){
    List<PopularModel> popularList = documents.map((data) => PopularModel.fromSnapshot(data)).toList();
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index){
        return _buildItem(popularList[index]);
      },
    );
  }

  Widget _buildItem(PopularModel items){
    return Container(
