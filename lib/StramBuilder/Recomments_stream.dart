...
import '../helper/recomment_product_helper.dart';
import '../model/recomment_product_model.dart';
import '../product/DetailPageForViewProduct.dart';


class RecommnetStream extends StatefulWidget {
  const RecommnetStream({Key? key}) : super(key: key);

  @override
  State<RecommnetStream> createState() => _RecommnetStreamState();
}

class _RecommnetStreamState extends State<RecommnetStream> {
  ...
  Widget _buildListView(List<DocumentSnapshot> documents){
    List<RecommentProductModel> recommentList = documents.map((data) => RecommentProductModel.fromSnapshot(data)).toList();
    return ListView.builder(
      physics: BouncingScrollPhysics(),
      scrollDirection: Axis.horizontal,
      itemCount: recommentList.length,
      itemBuilder: (context, index){
        return _buildItem(recommentList[index]);
      },
    );
  }
  Widget _buildItem(RecommentProductModel items){
    return Container(
            ...
            children: [
              Stack(
                children: [
                  ...
                  //       child: _isFavorite ? Icon(Icons.favorite,color: Colors.redAccent,) : Icon(Icons.favorite,color: Colors.white,),
                  //     ),
                  //   ),
                  // )
                ],
